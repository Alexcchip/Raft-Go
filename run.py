...
class Replica:
    ...
    def run(self, rids, silence):
        ...
        try:
            ...
            fail('*** Simulator Error - Unable to execute ./4730kvstore')
            self.alive = False
...
class Simulation:        
        ...
        while clock - start < self.conf.lifetime and self.stats.total_msgs < self.conf.max_packets and len(self.living_rids) > 0:
            ...
            for sock in ready:
                for r in self.replicas.values():
                    ...
                    if r.socket == sock:
                        self.__route_msgs__(r, sock)

                    # Otherwise its a output socket
                    elif sock in r.fds:
                        self.__print_output__(r, sock, start)
    ...
    def __print_output__(self, r, sock, start):
        ...
        if len(msg) > 0:
            ...
            self.__close_replica__(r)

    def __route_msgs__(self, r, sock):
